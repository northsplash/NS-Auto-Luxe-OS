...
function TrainingCenter({employees}:{employees:Employee[]}){
 ...
 const applyAcademy=async()=>{setApplying(true);const errs=await applyNewHireAcademy(employees);await load();setApplying(false);alert(errs.length?`Academy saved with a warning: ${errs[0]}`:`Door-to-door and detailing academies are live and assigned to matching team members.`)};
