...
function CommunicationsCenter(){
 ...
 const test=async()=>{if(!selected||!testEmail)return;const{data,error}=await supabase.functions.invoke('send-communication',{body:{event_key:selected.event_key,recipient_email:testEmail,variables:{customer_first_name:'Matthew',customer_name:'Matthew',detailer_name:'Jordan M.',employee_name:'Jordan M.',detailer_photo:'',service_name:'Luxe Signature Detail',vehicle:'2022 BMW 330i',vehicle_info:'2022 BMW 330i',appointment_time:new Date(Date.now()+86400000).toLocaleString(),address:'Raleigh, NC',price:'$275.00',amount:'$275.00',eta:'10:42 AM',portal_link:'https://ns-auto-luxe-os.vercel.app/portal'}}});if(error)return alert(error.message);alert(data?.success?'Test email sent.':data?.error||'Unable to send test.')};
 ...
 const sample=(text:string)=>String(text||'').replace(/{{?customer_first_name}?}/g,'Matthew').replace(/{{?customer_name}?}/g,'Matthew').replace(/{{?detailer_name}?}/g,'Jordan M.').replace(/{{?employee_name}?}/g,'Jordan M.').replace(/{{?vehicle(_info)?}?}/g,'2022 BMW 330i').replace(/{{?service(_name)?}?}/g,'Luxe Signature Detail').replace(/{{?appointment_time}?}/g,'Saturday, September 12 · 10:30 AM').replace(/{{?price}?}|{{?amount}?}/g,'$275.00').replace(/{{?eta}?}/g,'10:42 AM').replace(/{{?portal_link}?}/g,'northsplash.com/appointment');
...
function CommandCenter({employees,appointments,customers,payments}:{employees:Employee[];appointments:Appointment[];customers:Profile[];payments:any[]}){
 ...
 const unassigned=appointments.filter(a=>!a.assigned_employee_id&&!['completed','cancelled'].includes(a.status)).length,pending=appointments.filter(a=>a.status==='pending').length,qc=appointments.filter(a=>a.qc_status==='pending'||a.status==='qc').length;
 ...
 return <div className="tab-content phase300 v2-page owner-command-v17">
   ...
   <div className="owner-command-grid-v17">
    ...
    <section id="owner-schedule" className="phase-panel owner-schedule-v17"><div className="phase-panel-head"><div><span className="eyebrow">TODAY'S SCHEDULE</span><h3>{jobs.length} jobs</h3></div><button className="btn-outline btn-sm" onClick={()=>go('schedule')}>View all</button></div>{jobs.slice(0,6).map(j=><button className="owner-job-v17" key={j.id} onClick={()=>go('dispatch')}><time>{time(j.scheduled_at)}</time><span><b>{j.customer_name||j.service_address||'Customer'}</b><small>{j.vehicle_info||'Vehicle not added'}</small></span><span><b>{j.service_name}</b><small>{money(Number(j.price||0))}</small></span><span><small>{j.assigned_employee_id?employees.find(e=>e.id===j.assigned_employee_id)?.name||'Assigned':'Unassigned'}</small><b className={`status-badge badge-${j.status==='completed'?'green':j.status==='cancelled'?'red':'blue'}`}>{j.status.replaceAll('_',' ')}</b></span></button>)}{!jobs.length&&<div className="ns-empty">No appointments today. Your next scheduled job will appear here.</div>}</section>
