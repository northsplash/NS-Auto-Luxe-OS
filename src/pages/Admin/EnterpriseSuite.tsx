...
export default function EnterpriseSuite({ section, employees, setEmployees, appointments, setAppointments }: Props) {
  ...
  const d2dReps = employees.filter(employeeCanD2D);
      ...
      <section className="admin-card v19-assignment-shell">
        ...
        {activeJobs.length ? <div className="v19-assignment-table">
          ...
          {activeJobs.map(a => <div className="v19-assignment-row" key={a.id}>
            ...
            <select aria-label="Originating sales rep" value={a.sales_rep_employee_id || ''} onChange={e => assignJob(a.id,'sales_rep_employee_id',e.target.value)}><option value="">No rep</option>{d2dReps.map(e => <option key={e.id} value={e.id}>{leadRepLabel(e)}</option>)}</select>
