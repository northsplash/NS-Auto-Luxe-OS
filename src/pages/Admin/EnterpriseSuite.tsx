...
export default function EnterpriseSuite({ section, employees, setEmployees, appointments, setAppointments }: Props) {
  ...
  const importTerritoryHouses = async (territory: LeadTerritory) => {
    ...
    try {
      ...
      const elements=await fetchTerritoryHouses({south:Math.min(...lats),west:Math.min(...lngs),north:Math.max(...lats),east:Math.max(...lngs),points});
      const houses=mapOsmHouses(elements,points);
      const rows=houses.map(x=>({territory_id:territory.id,address:x.address,latitude:x.lat,longitude:x.lng,status:'unworked',source:'openstreetmap'})).slice(0,1200);
  ...
  if (section === 'job_assignments') {
    const activeJobs = appointments.filter(a => !a.archived && a.status !== 'cancelled');
    const unassigned = activeJobs.filter(a => !a.assigned_employee_id);
    const scheduledToday = activeJobs.filter(a => a.scheduled_at && new Date(a.scheduled_at).toDateString() === new Date().toDateString());
    const activeDetailers = employees.filter(e => employeeCanDetail(e) && e.status === 'active');
    return <div className="tab-content v19-job-assignment"><Header tab="job_assignments" />
      <div className="v19-kpi-grid">
        <div className="v19-kpi"><span className="v19-kpi-icon"><Briefcase size={20}/></span><div><strong>{activeJobs.length}</strong><span>Active Jobs</span><small>Not cancelled or archived</small></div></div>
        <div className="v19-kpi"><span className="v19-kpi-icon"><Users size={20}/></span><div><strong>{activeDetailers.length}</strong><span>Available Detailers</span><small>Active detailer profiles</small></div></div>
        <div className="v19-kpi"><span className="v19-kpi-icon"><ClipboardCheck size={20}/></span><div><strong>{scheduledToday.length}</strong><span>Scheduled Today</span><small>Today's workload</small></div></div>
        <div className={`v19-kpi ${unassigned.length ? 'v19-kpi-alert' : ''}`}><span className="v19-kpi-icon"><UserCog size={20}/></span><div><strong>{unassigned.length}</strong><span>Needs Assignment</span><small>{unassigned.length?'Action required':'All jobs assigned'}</small></div></div>
      </div>
      <section className="admin-card v19-assignment-shell">
        <div className="admin-card-header"><h3><UserCog size={18}/> Assignment Queue</h3><span className="v19-card-kicker">LIVE OPERATIONS</span></div>
        {activeJobs.length ? <div className="v19-assignment-table">
          <div className="v19-assignment-head"><span>Customer / Service</span><span>Detailer</span><span>Manager</span><span>Sales Rep</span><span>When</span></div>
          {activeJobs.map(a => <div className="v19-assignment-row" key={a.id}>
            <div className="v19-assignment-customer"><span className="v19-avatar">{customerName(a.user_id).slice(0,2).toUpperCase()}</span><div><strong>{customerName(a.user_id)}</strong><small>{a.service_name} · {money(a.price)}</small></div></div>
            <select aria-label="Assigned detailer" value={a.assigned_employee_id || ''} onChange={e => assignJob(a.id,'assigned_employee_id',e.target.value)}><option value="">Unassigned</option>{employees.filter(employeeCanDetail).map(e => <option key={e.id} value={e.id}>{e.name} · L{e.employment_level || 1}</option>)}</select>
            <select aria-label="Assigned manager" value={a.assigned_manager_id || ''} onChange={e => assignJob(a.id,'assigned_manager_id',e.target.value)}><option value="">No manager</option>{employees.filter(e => e.role === 'manager').map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select>
            <select aria-label="Originating sales rep" value={a.sales_rep_employee_id || ''} onChange={e => assignJob(a.id,'sales_rep_employee_id',e.target.value)}><option value="">No rep</option>{employees.filter(employeeCanD2D).map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select>
            <span className="v19-assignment-time">{dt(a.scheduled_at)}</span>
          </div>)}
        </div> : <div className="v19-premium-empty"><span className="v19-empty-icon"><CheckCircle2 size={28}/></span><h3>No jobs waiting for assignment</h3><p>New appointments will appear here automatically so you can assign a detailer, manager and originating rep.</p></div>}
      </section>
      <div className="v19-assignment-footer-grid"><section className="admin-card"><div className="admin-card-header"><h3>Detailer Capacity</h3></div><div className="v19-capacity-list">{activeDetailers.map(e=>{const count=activeJobs.filter(a=>a.assigned_employee_id===e.id).length;return <div key={e.id}><EmployeeAvatar employee={e} size="sm" className="v23-capacity-avatar"/><div><strong>{e.name}</strong><small>{count} active job{count===1?'':'s'}</small></div><i><b style={{width:`${Math.min(100,count*25)}%`}}/></i></div>})}{!activeDetailers.length&&<div className="v19-empty">No active detailers yet.</div>}</div></section><section className="admin-card"><div className="admin-card-header"><h3>Dispatch Notes</h3></div><div className="v19-dispatch-note"><strong>Assignment flow</strong><p>Appointments → assign detailer → confirm manager coverage → preserve the D2D rep attribution → send the job into Dispatch.</p></div></section></div>
    </div>;
  }
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="report-metric-card"><span>{label}</span><h2>{value}</h2></div>; }
...
function Permissions({ profiles, employees, setEmployees, onSave }: { profiles: Profile[]; employees: Employee[]; setEmployees: React.Dispatch<React.SetStateAction<Employee[]>>; onSave: (person: Profile, role: PortalRole, permissions: Record<string, boolean>) => Promise<void> }) {
  ...
  useEffect(()=>{if(person){const raw=person.role==='admin'?'owner':person.portal_role||'customer';const valid=(['owner','manager','employee','d2d','recruiter','finance','customer'] as PortalRole[]).includes(raw as PortalRole)?raw as PortalRole:'customer';setRole(valid);setPerms(person.role==='admin'?{...DEFAULT_ROLE_PERMISSIONS.owner}:{...(DEFAULT_ROLE_PERMISSIONS[valid]||{}),...(person.permissions||{})});setLinkEmployeeId(employees.find(e=>e.user_id===person.id)?.id||employees.find(e=>e.email?.toLowerCase()===person.email?.toLowerCase())?.id||'');}},[person?.id,employees]);
  ...
  return <div className="tab-content permissions-v3">
    ...
    <div className="permissions-layout-v3">
      <aside className="permission-accounts">
        ...
        <div className="permission-account-list">{profiles.map(p=><button key={p.id} onClick={()=>setSelected(p.id)} className={p.id===selected?'selected':''}><EmployeeAvatar profileId={p.id} name={p.full_name||p.email||'Account'} avatarUrl={p.avatar_url} size="sm" className="permission-avatar v23-profile-avatar"/><span><strong>{p.full_name||p.email||'Account'}</strong><small>{p.email}</small><em>{roleLabel(p.role==='admin'?'owner':p.portal_role)}</em></span></button>)}</div>
      </aside>
