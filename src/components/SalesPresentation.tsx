...
import {
  generateDoorPassword,
  householdFromSeed,
  patchHousehold as mergeHousehold,
  portalLoginUrl,
  type ApplyOfferResult,
  type DoorCustomerAccount,
  type OfferSelection,
  type PresentationHousehold,
} from '@/lib/customerAccount';

export type { OfferSelection, PresentationHousehold };
...
type Props = {
  ...
  householdSeed?: Partial<PresentationHousehold>;
  leadId?: string | null;
  onClose?: () => void;
  onSelectOffer?: (offer: OfferSelection) => void;
  onApplyAndSave?: (offer: OfferSelection) => void | Promise<ApplyOfferResult | void>;
...
export default function SalesPresentation({
  customerName, customerPhone, customerEmail, customerAddress, householdSeed, leadId,
  ...
  const [household, setHousehold] = useState<PresentationHousehold>(() => householdFromSeed({
    ...householdSeed,
    name: householdSeed?.name || customerName,
    phone: householdSeed?.phone || customerPhone,
    email: householdSeed?.email || customerEmail,
    address: householdSeed?.address || customerAddress,
  }));
  const [createPortal, setCreatePortal] = useState(true);
  const [sendInvite, setSendInvite] = useState(true);
  const [password, setPassword] = useState(generateDoorPassword);
  const [showPw, setShowPw] = useState(true);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  const [copied, setCopied] = useState('');
  const [accountResult, setAccountResult] = useState<DoorCustomerAccount | null>(null);
  ...
  const currentOffer: OfferSelection = {
    type: offerType,
    name: offerType === 'service' ? pkg.name : membership.name,
    amount: offerType === 'service' ? servicePrice : membership.price,
    detail: offerType === 'service' ? `${vehicle.name} · starting estimate` : 'monthly membership',
    household,
    createPortalAccount: createPortal,
  ...
  const loginUrl = accountResult?.login_url || portalLoginUrl();

  const patchHousehold = (patch: Partial<PresentationHousehold>) => setHousehold((p) => mergeHousehold(p, patch));
  ...
  const applyNow = async (offer: OfferSelection) => {
    ...
    if (offer.createPortalAccount) {
      if (!offer.household.email.trim()) {
        setError('Email is required to open their customer portal login.');
        setMode('account');
        return;
      }
      if (offer.portalPassword.trim().length < 8) {
        setError('Portal password must be at least 8 characters.');
        setMode('account');
        return;
      }
    } else if (!householdReady) {
      setError('Add a name, phone, or street before saving.');
      setMode('account');
      return;
    }
    setBusy(true);
    setError('');
    try {
      onEvent?.('offer_selected', { ...offer, save: true, lead_id: leadId || null });
      const result = await onApplyAndSave(offer);
      ...
      setError(err instanceof Error ? err.message : 'Unable to apply this account.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const chooseOffer = (save = false) => {
    ...
    if (save && onApplyAndSave) { void applyNow(currentOffer); return; }
  ...
  const accountSuccess = accountResult ? (
    <div className="sales-account-success">
      <span className="sales-kicker">CUSTOMER PORTAL READY</span>
      <h2>{accountResult.created ? 'Their login is live.' : 'Existing customer login updated.'}</h2>
      <p>They sign in at the customer portal with this email and password. Your D2D session stays signed in — this did not switch accounts.</p>
      <dl>
        <div><dt>Portal</dt><dd>{loginUrl}<button type="button" onClick={() => void copyText(loginUrl, 'link')}><Copy />{copied === 'link' ? 'Copied' : 'Copy'}</button></dd></div>
        <div><dt>Email</dt><dd>{accountResult.email}<button type="button" onClick={() => void copyText(accountResult.email, 'email')}><Copy />{copied === 'email' ? 'Copied' : 'Copy'}</button></dd></div>
        {accountResult.password && <div><dt>Password</dt><dd>{accountResult.password}<button type="button" onClick={() => void copyText(accountResult.password || '', 'password')}><Copy />{copied === 'password' ? 'Copied' : 'Copy'}</button></dd></div>}
      </dl>
      {accountResult.emailed && <p className="sales-account-note">A welcome email with a reset link is on the way.</p>}
      <button type="button" className="sales-use-offer" onClick={() => onClose?.()}>Done</button>
    </div>
  ) : null;
  ...
  const tree = <div className={`nsos-cream ${embedded ? 'sales-presentation-embedded' : 'sales-presentation-overlay'}`}>
    <div className="sales-presentation-shell">
      ...
      {mode === 'account' && <div className="sales-quote-mode sales-account-mode">
        {accountSuccess || <>
          <section className="sales-quote-builder">
            <div className="sales-slide-heading">
              <span className="sales-kicker">CUSTOMER ACCOUNT</span>
              <h2>Open their portal at the door.</h2>
              <p>Capture the household the way SalesRabbit would, then create a customer login they can use after you leave. Name and phone save a lead. Email plus a password opens the portal.</p>
            </div>
            <div className="sales-household-grid">
              ...
              <label><span>Street 2</span><input autoComplete="address-line2" placeholder="Apt / unit" value={household.street2} onChange={(e) => patchHousehold({ street2: e.target.value })} /></label>
              <label><span>Vehicle</span><input placeholder="Year / make / model" value={household.vehicle} onChange={(e) => patchHousehold({ vehicle: e.target.value })} /></label>
              <div className="sales-addr-row">
                <label><span>City</span><input autoComplete="address-level2" placeholder="City" value={household.city} onChange={(e) => patchHousehold({ city: e.target.value })} /></label>
                <label><span>State</span><input autoComplete="address-level1" placeholder="NC" value={household.state} onChange={(e) => patchHousehold({ state: e.target.value })} /></label>
                <label><span>ZIP</span><input autoComplete="postal-code" inputMode="numeric" placeholder="ZIP" value={household.postal_code} onChange={(e) => patchHousehold({ postal_code: e.target.value })} /></label>
              </div>
            </div>
          </section>
          <aside className="sales-quote-summary">
            <span className="sales-kicker">PORTAL LOGIN</span>
            <h3>{household.name || 'New customer'}</h3>
            <p>{createPortal ? 'Creates a customer account without signing you out of D2D.' : 'Saves the household on this door only — no portal login yet.'}</p>
            <label className="sales-check"><input type="checkbox" checked={createPortal} onChange={(e) => setCreatePortal(e.target.checked)} /><span>Create their customer portal login</span></label>
            {createPortal && <>
              <label className="sales-password-field">
                <span>Password they will use</span>
                <div className="sales-password-row">
                  <input type={showPw ? 'text' : 'password'} autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} />
                  <button type="button" className="sales-icon-btn" onClick={() => setShowPw((v) => !v)} title={showPw ? 'Hide password' : 'Show password'}>{showPw ? <EyeOff /> : <Eye />}</button>
                  <button type="button" className="sales-icon-btn" onClick={() => setPassword(generateDoorPassword())} title="Generate a new password"><KeyRound /></button>
                </div>
              </label>
              <label className="sales-check"><input type="checkbox" checked={sendInvite} onChange={(e) => setSendInvite(e.target.checked)} /><span>Email a portal welcome / reset link</span></label>
            </>}
            {error && <p className="sales-account-error" role="alert">{error}</p>}
            {onApplyAndSave && <button type="button" className="sales-use-offer" disabled={busy} onClick={() => {
              void applyNow({ ...currentOffer, name: '', amount: 0, detail: 'Customer account' });
            }}><UserRound />{busy ? 'Applying…' : createPortal ? 'Apply customer account' : 'Save household only'}</button>}
            <button type="button" className="sales-save-offer" onClick={() => setMode('quote')}><Target />Continue to quote</button>
            <button type="button" className="sales-back-presentation" onClick={() => setMode('presentation')}><ArrowLeft />Back to Presentation</button>
            <small>Apply writes the household onto this door and, when checked, opens a customer login at {loginUrl.replace(/^https?:\/\//, '')}.</small>
          </aside>
        </>}
      </div>}
      {mode === 'quote' && <div className="sales-quote-mode">
        <section className="sales-quote-builder"><div className="sales-slide-heading"><span className="sales-kicker">CUSTOMER QUOTE</span><h2>Build the offer in front of the customer.</h2><p>Select a one-time service or membership, then apply it with their customer account. Final pricing can still be adjusted for condition and add-ons.</p></div>
          <div className="sales-household-inline">
            <label><span>First name</span><input placeholder="First" value={household.first_name} onChange={(e) => patchHousehold({ first_name: e.target.value })} /></label>
            <label><span>Last name</span><input placeholder="Last" value={household.last_name} onChange={(e) => patchHousehold({ last_name: e.target.value })} /></label>
            ...
            <label><span>Email</span><input type="email" placeholder="Portal email" value={household.email} onChange={(e) => patchHousehold({ email: e.target.value })} /></label>
          </div>
          ...
          {offerType === 'service' ? <><DetailSelfPicker tone="cream" family={family} self={self} onChange={(nextFamily, nextSelf) => { setFamily(nextFamily); setSelf(nextSelf); }} /><div className="sales-quote-options"><label><span>Vehicle size</span><select value={vehicleIndex} onChange={(e) => setVehicleIndex(Number(e.target.value))}>{VEHICLE_SIZES.map((v, i) => <option value={i} key={v.name}>{v.name}{v.extra ? ` +${money(v.extra)}` : ''}</option>)}</select></label></div></> : <div className="sales-quote-plan-picker">{MEMBERSHIPS.map((m, i) => <button type="button" className={i === membershipIndex ? 'active' : ''} key={m.name} onClick={() => setMembershipIndex(i)}><span>{m.name}</span><strong>{money(m.price)}<small>/mo</small></strong><em>{m.desc}</em></button>)}</div>}
        </section>
        <aside className="sales-quote-summary"><span className="sales-kicker">TODAY'S RECOMMENDATION</span><h3>{currentOffer.name}</h3><strong>{money(currentOffer.amount)}<small>{currentOffer.type === 'membership' ? '/month' : ' estimated'}</small></strong><p>{currentOffer.detail}{household.name ? ` · ${household.name}` : ''}</p><div className="sales-quote-includes">{(currentOffer.type === 'service' ? pkg.features : membership.features).slice(0, 6).map((f) => <span key={f}><Check />{f}</span>)}</div>
          <label className="sales-check"><input type="checkbox" checked={createPortal} onChange={(e) => setCreatePortal(e.target.checked)} /><span>Also create their customer portal login</span></label>
          {error && <p className="sales-account-error" role="alert">{error}</p>}
          ...
          {onApplyAndSave && <button type="button" className="sales-save-offer" disabled={busy} onClick={() => chooseOffer(true)}><UserRound />{busy ? 'Applying…' : createPortal ? 'Apply account & save' : householdReady ? 'Apply & save this household' : 'Apply & save lead'}</button>}
          <button type="button" className="sales-back-presentation" onClick={() => setMode('account')}>Open account details</button>
          <button type="button" className="sales-back-presentation" onClick={() => setMode('presentation')}><ArrowLeft />Back to Presentation</button>
          <small>{createPortal ? 'Apply creates their portal login, writes the offer onto this door, and keeps you signed in as the rep.' : 'Applying writes the package onto the D2D lead. Open Account to add a portal login.'}</small>
        </aside>
      </div>}
