...
export default function DispatchCommandCenter({employees,appointments,setAppointments}:Props){
  ...
  useEffect(()=>{
    const filter=takeOwnerBoardFilter();
    if(filter==='unassigned')setMode('people');
    if(filter==='run'||filter==='collect')setMode('status');
  },[]);
