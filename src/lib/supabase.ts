...
export type Appointment = {
  ...
  status: 'pending' | 'scheduled' | 'confirmed' | 'en_route' | 'arrived' | 'in_progress' | 'completed' | 'cancelled' | 'no_show';
  price: number;
  notes: string | null;
  archived?: boolean;
...
export type Employee = {
  ...
  pay_type?: 'hourly' | 'salary' | 'base_commission' | 'commission_only' | 'per_job' | 'custom' | string;
  hourly_rate?: number;
  weekly_base?: number;
  ...
  commission_basis?: 'revenue' | 'gross_profit' | 'job' | 'membership' | string;
  pay_schedule?: 'weekly' | 'biweekly' | 'semimonthly' | 'monthly' | string;
  overtime_eligible?: boolean;
  compensation_notes?: string | null;
  custom_compensation?: Record<string, unknown> | null;
