-- V35: Gusto payroll fields on the hire packet (Personal details, W-4 2020, NC-4, payment method, I-9, emergency).
...
comment on column public.employee_onboarding_profiles.ohio_filing_status is
  'NC-4 filing status. Column name is historical; values are North Carolina, not Ohio.';
comment on column public.employee_onboarding_profiles.ohio_school_district is
  'NC work county (e.g. Wake County). Column name is historical.';
comment on column public.employee_onboarding_profiles.ohio_extra_withholding is
  'Extra NC withholding. Column name is historical.';
comment on column public.employee_onboarding_profiles.gusto is
  'Gusto payroll extras (W-4 2020, NC-4, payment method). Full SSN and bank numbers are never stored.';
